export default function Service() {
  return (
    <section className="service">
      <div className="container">
        <h1 className="display-4 section-title">Services</h1>
        <p className="lead text-muted">
          Lorem ipsum dolor sit amet consectetur adipisicing elit. Saepe quam,
          consequuntur eaque accusantium rerum explicabo, deleniti voluptate,
          excepturi velit perferendis sequi quod consequatur aperiam ipsum
          possimus error harum repellendus neque!
        </p>

        <div className="card-grid">
          <div className="service-card">
            <h2 className="service-card-header">First card</h2>

            <div className="service-card-text">
              <p>
                Lorem ipsum dolor sit amet consectetur, adipisicing elit.
                Deleniti, soluta voluptatem adipisci dignissimos explicabo
                veritatis, vero sit at dolore autem quibusdam, ea qui minus
                dolorem totam natus perferendis eum officia.
              </p>
            </div>
          </div>

          <div className="service-card">
            <h2 className="service-card-header">Second card</h2>

            <div className="service-card-text">
              <p>
                Lorem ipsum dolor sit amet consectetur, adipisicing elit.
                Deleniti, soluta voluptatem adipisci dignissimos explicabo
                veritatis, vero sit at dolore autem quibusdam, ea qui minus
                dolorem totam natus perferendis eum officia.
              </p>
            </div>
          </div>

          <div className="service-card">
            <h2 className="service-card-header">Third card</h2>

            <div className="service-card-text">
              <p>
                Lorem ipsum dolor sit amet consectetur, adipisicing elit.
                Deleniti, soluta voluptatem adipisci dignissimos explicabo
                veritatis, vero sit at dolore autem quibusdam, ea qui minus
                dolorem totam natus perferendis eum officia.
              </p>
            </div>
          </div>
        </div>
      </div>
    </section>
  );
}
